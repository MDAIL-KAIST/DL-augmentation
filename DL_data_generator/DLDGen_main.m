...
% 
DLDgen.target_cell_vector_a = [3.912,  0.000,  0.000]; % cell vector
DLDgen.target_cell_vector_b = [0.000,  3.912,  0.000];
DLDgen.target_cell_vector_c = [0.000,  0.000,  3.912];
DLDgen.target_init_atoms = [78 78 78 78];       % atoms
DLDgen.target_init_basis = [0.0  0.5  0.0  0.5;  % basis
                            0.0  0.5  0.5  0.0;
                            0.0  0.0  0.5  0.5];

% random parameters for target data
DLDgen.target_Random_shape = 1; % crop shape randomly on/off
DLDgen.target_Random_defect = 0; % vacancy defect percent
DLDgen.target_Random_rotation = 180; % degree
DLDgen.target_Random_shift = 0.22; % angstrom
DLDgen.target_Random_traslation = 1; % pixel
DLDgen.target_Random_shuffling_atomtype = 0;
...
DLDgen.GENFIRE.griddingMethod = 2;  % 1 for FFT, 2 for DFT
