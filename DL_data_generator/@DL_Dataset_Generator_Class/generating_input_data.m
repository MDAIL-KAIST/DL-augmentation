...
input_Projs_method = obj.input_Projs_method; % projection option
input_Projs_GPU = obj.input_Projs_GPU; % GPU boost up
...
dt = datestr(now,'yyyy_mm_dd-HH_MM_SS_FFF');
...
for i=Number_data_start:1:Number_data_final
    ...
    if input_Projs_method == 1
        Car_Projs = zeros(size(Vol,1)*2,size(Vol,2)*2,length(anglelist));
        Vol1=My_paddzero(Vol,[size(Vol,1),size(Vol,2),size(Vol,3)]*2);
        
        if input_Projs_GPU == 1
            Car_Projs = gpuArray(single(Car_Projs));
            Vol1=gpuArray(single(Vol1));
        end
        ...
        Car_Projs=My_stripzero(Car_Projs,[size(Vol,1),size(Vol,2),length(anglelist)]);

        if input_Projs_GPU == 1
            Car_Projs=gather(Car_Projs);
        end
        
        clear Vol_k Vol1
        
    elseif input_Projs_method == 2
        ...
        for angle = anglelist
            theta = angle; % input
            Car_Projs(:,:,j) = Func_calculate3Dprojection_car(Vol1,phi,theta,psi,vec1,vec2,vec3);
            j = j+1;
        end
    ...
    %GENFIRE.vector1 = obj.GENFIRE.vector1;
    %GENFIRE.vector2 = obj.GENFIRE.vector2;
    GENFIRE.vector3 = obj.GENFIRE.vector3;
