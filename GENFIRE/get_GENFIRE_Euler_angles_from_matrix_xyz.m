...
function new_Angles = get_GENFIRE_Euler_angles_from_matrix_xyz(rotMAT,varargin)
...
if abs(rotMAT(3,1))>1
    rotMAT(3,1) = sign(rotMAT(3,1))*1;
end

new_Angles(2) = -1*asind(rotMAT(3,1));

if abs(cosd(new_Angles(2))) > zero_tolerance
    new_Angles(1) = atan2(rotMAT(2,1),rotMAT(1,1))*180/pi;
    new_Angles(3) = atan2(rotMAT(3,2),rotMAT(3,3))*180/pi;
else
    new_Angles(1) = atan2(rotMAT(2,3)*rotMAT(3,1),rotMAT(1,3)*rotMAT(3,1))*180/pi;
    new_Angles(3) = 0;
end
