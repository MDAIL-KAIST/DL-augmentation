function [minAngle12_1, minAngle12_2] = get_CMline_bestInpAngles_LS_parallel_precom_magonly(PreCMline1,PreCMline2,PreAngles,Angle1,Angle2,Range)
...
parfor i=1:length(searchInds1)
  ...
  for j=1:length(searchInds2)
    ...
    Errar(j) = sum(abs(abs(CMline1F)-abs(CMline2F)).^2);
  end
  TotErrar(i,:) = Errar;
end
