function [d1, d2, BGforAdd, minVal] = My_Rscan_subpixel_align_img1mask_bgFit(img1,img2,RscanRad,Rres)
    ...
    for i=-radR:radR
        for j=-radR:radR
            ...
            shift_img2(~imgMask) = 0;                        
            
            [p,~,~,~] = lsqcurvefit(fun,[0],shift_img2,img1,[],[],opts);
            
            currR = calcR_norm_YY(img1,fun(p,shift_img2));
    ...
    for i=1:NumIter
        ...
        minPar = zeros(length(curr_ind1arr),length(curr_ind2arr));
        for j=1:length(curr_ind1arr)
            for k=1:length(curr_ind2arr)
                ...
                [p,~,~,~] = lsqcurvefit(fun,[0],shift_img2,img1,[],[],opts);          
                minRar(j,k) = calcR_norm_YY(img1,fun(p,shift_img2));
                minPar(j,k) = p;
            end
        end
