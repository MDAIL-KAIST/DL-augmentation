...
function [projection, kSlice] = calculate3Dprojection_interp_AJ(modelK,phi,theta,psi)
...
rotKx = reshape(rotKx,size(X)) + ncx;
rotKy = reshape(rotKy,size(Y)) + ncx;
rotKz = reshape(rotKz,size(Z)) + ncx;

%calculate points on central slice
% kSlice = interp3(ky,kx,kz,modelK,rotKy,rotKx,rotKz,'linear');
kSlice = splinterp3(modelK,rotKy,rotKx,rotKz);
