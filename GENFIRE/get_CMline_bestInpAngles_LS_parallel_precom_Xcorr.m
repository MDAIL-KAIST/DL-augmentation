function [minAngle12_1, minAngle12_2] = get_CMline_bestInpAngles_LS_parallel_precom_Xcorr(PreCMline1,PreCMline2,PreAngles,Angle1,Angle2,Range)
...
for i=1:length(searchInds1)
  Errar = zeros(1,length(searchInds2));
  for j=1:length(searchInds2)
      
    CMline1 = real(fftshift(ifft(ifftshift(PreCMline1(searchInds1(i),:)))));
    CMline2 = real(fftshift(ifft(ifftshift(PreCMline2(searchInds2(j),:)))));
    XX = xcorr(CMline1,CMline2);
    Errar(j) = max(XX);
  end
  TotErrar(i,:) = Errar;
end
